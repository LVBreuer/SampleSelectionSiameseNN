...
names = ['Random Offline Pair', 'Hard Negative Online Pair', 'All Positive Online Pair', 
                  'Random Offline Triplet', 'All Positive Online Triplet', 'Random Negative Online Triplet', 'Semi-hard Negative Online Triplet']
...
def train():
    ...
    # choosing the sample selection method and start training
    if selection == 0:
        model, training_loss, validation_loss, training_acc, validation_acc = SiameseTrain(train_dataset, test_dataset)
    elif selection == 3:
        model, training_loss, validation_loss, training_acc, validation_acc = TripletNetTrain(train_dataset, test_dataset)
    elif selection == 1 or selection == 2:
        model, training_loss, validation_loss, training_acc, validation_acc = OnlinePairSelection(train_dataset, test_dataset, selection)
    else:
        model, training_loss, validation_loss, training_acc, validation_acc = OnlineTripletSelection(train_dataset, test_dataset, selection)
    
    # draw results
    draw_training(test_dataset, model, training_loss, validation_loss, training_acc, validation_acc, names[selection], dataset, save_dir)
...
def test():
    ...
    # choosing the sample selection method and run the inference
    if selection == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--selection', default=0, type=int, help='0 = Random Offline Pair \n1 = Hard Negative Online Par \n2 = All Positive Online Pair \n3 = Random Online Triplet \n4 = All Positive Online Triplet \n5 = Random Negative Online Triplet \n6 = Semi-hard Negative Online Triplet')
