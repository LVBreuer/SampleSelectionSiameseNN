...
def load_train_dataset(dataset):
    # load only 9 classes from the trianing set for training the model
...
def load_train_dataset10(dataset):
    # load training dataset with all 10 classes to print out the results
    if dataset == 'FashionMNIST':
        train_dataset_ = FashionMNIST('./data/FashionMNIST10', train=True, download=True,
                                     transform=transforms.Compose([
                                         transforms.ToTensor(),
                                         transforms.Normalize((mean,), (std,))
                                     ]))
